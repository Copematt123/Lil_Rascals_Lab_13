...
#arrange by year which counntry spends the least and most
...
names(HSPP) <- c("Country", "Year", "SPC")
    ...
    cont1 <- HSPP %>% filter(Country == "United States") 
    cont1
    cont2 <- HSPP %>% filter(Country == "Luxembourg") 
    cont2
length(cont1$SPC)
    rcont1 <- sample(cont1, length(cont1$SPC)-15, replace = FALSE)
    rcont1
    rcont2 <- sample(cont2, length(cont2$SPC)-15, replace = FALSE)
    rcont2
    A <- mean(cont1$SPC)
    A
    B <- mean(cont2$SPC)
